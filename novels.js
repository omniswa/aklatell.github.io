(function() {
  'use strict';
  // IIFE to set initial state prevents FOUC (Flash of Unstyled Content)
  const savedTheme = localStorage.getItem('reader_theme') || 'light';
  const savedFont = localStorage.getItem('reader_font') || 'Newsreader';
  const savedSize = localStorage.getItem('reader_size') || 'medium';
  
  const root = document.documentElement;
  const sizeMap = { small: '1rem', medium: '1.125rem', large: '1.3rem' };
  
  root.setAttribute('data-theme', savedTheme);
  root.setAttribute('data-font', savedFont.toLowerCase());
  root.style.setProperty('--font-size-base', sizeMap[savedSize]);
})();
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const updateUI = () => {
    ...
    document.documentElement.setAttribute('data-font', state.font.toLowerCase());
    const sizeMap = { small: '1rem', medium: '1.125rem', large: '1.3rem' };
    document.documentElement.style.setProperty('--font-size-base', sizeMap[state.size]);

    // Update active buttons
    document.querySelectorAll('.theme-btn').forEach(b => b.classList.toggle('active', b.dataset.val === state.theme));
    document.querySelectorAll('.font-btn').forEach(b => b.classList.toggle('active', b.dataset.val === state.font));
    document.querySelectorAll('.size-btn').forEach(b => b.classList.toggle('active', b.dataset.val === state.size));
    ...
    // Recalculate height after style change
    calculateDocHeight();
  };

  settingsPanel.addEventListener('click', (e) => {
    if (e.target.classList.contains('theme-btn')) { state.theme = e.target.dataset.val; updateUI(); }
    if (e.target.classList.contains('font-btn')) { state.font = e.target.dataset.val; updateUI(); }
    if (e.target.classList.contains('size-btn')) { state.size = e.target.dataset.val; updateUI(); }
  });
  ...
  const handleScroll = () => {
    ...
    // Auto-hide UI logic (UX Improvement)
    if (scrollTop > lastScrollTop && scrollTop > 100) {
      uiControls.classList.add('hidden');
      settingsPanel.classList.remove('active'); // Close settings if scrolling
    } else {
  ...
  if (savedProgress > 0) {
    ...
    document.getElementById('toast-yes').addEventListener('click', () => {
      calculateDocHeight();
      const target = (savedProgress / 100) * docHeight;
